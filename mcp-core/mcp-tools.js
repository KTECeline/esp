      ...
      const SpcPanelInput = z
        .object({
          ...
          font_size: z
            ...
            .optional()
            .describe(
              "Starting text size for mode=message, before the panel auto-shrinks to fit. " +
              "Omit for \"medium\". A message that doesn't fit at its starting size shrinks " +
              "automatically, one step at a time, until every word is on screen — this only " +
              "picks where that shrink starts. Use \"large\" for a short line meant to be read " +
              "from across the room; use \"small\" to leave more headroom before a long answer " +
              "has to shrink. Ignored by qr, choices, order and blank."
            )
        })
        .strict();
