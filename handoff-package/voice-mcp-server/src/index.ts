...
const WHISPER_DIR = process.env.WHISPER_DIR || path.join(homedir(), "whisper.cpp");
...
server.registerTool(
  ...
  async (params: TranscribeInput) => {
    try {
      const args = ["-m", WHISPER_MODEL, "-f", params.audio_file_path, "-ng", "-nt", "-l", params.language || "auto"];
...
server.registerTool(
  ...
  async (params: SpeakInput) => {
    try {
      ...
      const outputPath = path.join(tempDir, "reply.mp3");
